...
Status: **Steps 1–4 implemented and deployed to production on 2026-09-04. Steps 5–6 are implemented and verified locally but not deployed. Steps 7–8 are not implemented.** All three production database migrations passed before the Steps 1–4 application deployment. The current Step 6 regression suite passes 681/681. See §17–18 for the production release §19 for Step 5, and §20 for Step 6 behavior and verification. Earlier local-only and blocker statements below describe their respective checkpoints.
...
| `POST /api/contacts/import` with `mode: "preview"` | Step 6. Write guard; parsed name/phone entries plus country/source. Server validation and account-scoped existing-contact lookup only. Raw file parsing, mapping, duplicate detection and row selection happen in memory in the browser. |
| `POST /api/contacts/import` | Step 6. Write guard; maximum 250 reviewed entries per transaction, supported source, and only permitted import fields. Revalidate everything. Return per-row outcomes and added/existing/rejected totals. Step 7 will extend the source allowlist for the picker. |
...
Implemented Step 6 limits: raw file 2 MiB, at most 2,000 source rows and candidate phone entries per preview, 200 CSV columns, commits of at most 250 entries per database transaction, and 256 KiB of streamed JSON per preview/commit request. These deliberately smaller pilot limits supersede the original 5 MiB / 10,000-entry proposal. Reject oversize inputs before unbounded parsing. Preview lists invalid rows, duplicate numbers, and conflicting names. New rows default unclassified/suppressed; explicitly selected rows may be Personal. Commit revalidates and rechecks existing contacts, so a stale preview cannot override current owner decisions.
...
## 20. Step 6 implementation and verification

Implemented locally on `codex/known-contacts`. No production data changes, deployment, or real messages were performed. No schema or migration change is needed: imports use the existing transactional `merge_known_contacts` RPC and its account/phone uniqueness and RLS protections.

### Formats and owner flow

- Settings offers **Import contacts → Choose file → Preview → Review → Import → Result** for owners/admins. CSV mapping is always reviewable, including when headers are recognized; unknown phone columns require explicit mapping. A country selector defaults to US and governs national numbers. Mixed-country files need explicit international + prefixes. Opening import disables competing add/edit controls so those actions cannot discard an active import session.
- `lib/contact-import.ts` implements a narrow, bounded parser rather than adding a general address-book dependency. Supported input is UTF-8 comma-separated CSV with doubled-quote escaping, CRLF/LF records, quoted multiline fields, optional BOM, Google numbered phone-value columns and `:::` multi-values; and UTF-8 vCard 3.0/4.0 with grouped TEL properties, multiple numbers, folded lines, escaped names, and simple `tel:` URIs. Fixtures are in `tests/fixtures/contacts/`. Older vCards, encoded names/phones, extensions/URI parameters, malformed quoting/cards and invalid numbers are rejected or flagged, never repaired by guessing. Multiline notes are ignored; names containing control characters are flagged under the existing name contract.
- Preview pages contain 20 entries and show original and normalized phone, name, invalid reason, duplicate/name conflict, and the proposed new/existing action. Defaults choose one valid occurrence per number, preferring the first nonblank name. Selecting another duplicate replaces the earlier selection; owners can resolve conflicts or exclude rows. Existing saved names, classification, and automatic-text preferences are displayed and retained. Only explicit individual selections mark new entries Personal. Other new entries are unclassified/suppressed.
- Raw files never reach the server. Browser CSV data is discarded after preview, and all remaining candidate state is cleared on close/new file. No local storage, upload persistence, address-book logging, or new analytics was added. The importer is blocked from Sentry session replay. Inspection of the installed Sentry SDK showed that the existing empty `dataCollection` object enabled HTTP-body and database-value capture; client/server/edge configuration now explicitly disables those two payload categories so import contents cannot be collected by those diagnostics. This applies to all routes while retaining error and performance monitoring. A focused initialization test verifies all three configurations. Only phone/name/classification/source and existing operational metadata reach storage; notes, emails, addresses, birthdays, and photos are ignored. Field text is rendered through React as text, never HTML or spreadsheet formulas.

### API, transactions, and retry contract

`POST /api/contacts/import` accepts `{mode: "preview" | "import", source: "csv" | "vcard", country, entries: [{phone, displayName?, classification?}]}`. It derives account scope from the existing owner/admin write guard and rejects extra top-level fields. The bounded streamed reader enforces 256 KiB even without a trustworthy Content-Length header; requests allow 1–250 entries. Each row is independently validated on the server. Import classifications are limited to Personal or unclassified; automatic-text policy fields and Customer classification cannot be imported.

Preview returns account-scoped existing contacts and indexed rejected rows without writing. Import sends valid rows through the shared merge service and returns canonical per-number `outcomes`, `added`, `existing`, `duplicates`, and indexed `rejected` reasons. Duplicate conflicting classifications cause the merge to reject the batch. Existing rows are unchanged, even if edited after preview. New rows always suppress automation. Neither endpoint creates leads or sends messages.

Each valid batch is one atomic database transaction. Invalid rows are rejected before that transaction, so valid rows can still import. Completed batch offsets and confirmed counts remain in browser memory. A failed/uncertain batch stops further processing and exposes **Retry remaining import**; the selection stays fixed after a partial or uncertain commit. Retrying an uncertain committed batch reports its entries as existing, accurately labeled in the result instead of claiming they were newly added. Reloading loses the session ledger; reselecting the same file and importing again safely preserves current contacts. No total-new-in-original-attempt claim is made when an acknowledgement was lost.

### Verification

| Check | Result and limits |
| --- | --- |
| Full repository tests | **681 passed**, zero failures/skips. Nine new behavioral tests cover representative formats, quoting/folding/escaping/multiple numbers, malformed inputs/limits, explicit mapping, untrusted names, duplicate and cross-format imports, lost-response retry, owner-decision preservation, tenant scoping, viewer denial, forbidden policy fields, and streamed byte enforcement. |
| Local PostgreSQL | **30 passed**, including fresh/upgrade/repeated schema checks, RLS/grants, transactional merges, concurrent/repeated inserts, tenant isolation, retained history, and classification rules. The existing private socket-only cluster was restarted; sandbox socket access required escalation. No new migration was necessary. |
| Lint | Passed with no warnings. |
| Typecheck | Isolated application copy passes. Direct workspace typecheck remains blocked solely by the previously documented unrelated untracked `app/api/twilio/sms/route 2.ts` missing `notificationId` arguments; those files were preserved. |
| Production build | Isolated full application build passed with synthetic settings and no production credentials. Font fetching required network access outside the sandbox. |
| Desktop UI | Actual React/Next import component in an isolated local harness at **1280×900**, synthetic API only: file chooser, Google CSV mapping, duplicate replacement, invalid row exclusion, review, and result (1 added/1 existing) verified. |
| Mobile UI | Same component at **390×844**, synthetic API only: Apple vCard preview, retained existing Customer preference, explicit individual Personal selection, review and result (2 added/1 existing). Screenshots inspected; document width equals viewport width (390px) in preview and result. Step changes focus their heading; mapping inputs have column-specific accessible names. |

Browser API responses were synthetic; real SQL behavior was independently exercised by the database suite. Physical phone file pickers and real Android/iPhone devices have not been tested. Step 7 remains native contact-picker support; Step 8 remains final integration/pilot preparation. Step 6 is not deployed.
