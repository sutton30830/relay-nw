...
export async function POST(request: Request) {
  ...
  if (!account) redirect(`/ops?account=${encodeURIComponent(accountSlug)}&activation=account_not_found`);
  if (account.onboardingStatus !== "ready_to_activate") redirect(`/ops?account=${encodeURIComponent(account.accountSlug)}&activation=blocked`);
  ...
  const update = action === "comp"
    ...
    : action === "trial"
      ...
      : null;
  if (!update) redirect(`/ops?account=${encodeURIComponent(account.accountSlug)}&activation=invalid_action`);
  ...
  redirect(`/ops?account=${encodeURIComponent(account.accountSlug)}&activation=${action}`);
}
