...
-- Reapplying the schema must never silently grant or restore Operations access.
...
create table if not exists public.leads (
  ...
  sms_status text check (sms_status in ('pending', 'queued', 'sending', 'sent', 'delivered', 'failed', 'undelivered', 'skipped_disabled', 'skipped_opt_out', 'skipped_recent', 'skipped_known_contact', 'blocked_pre_send')),
  sms_error text,
...
alter table public.leads
  add constraint leads_sms_status_check check (sms_status in ('pending', 'queued', 'sending', 'sent', 'delivered', 'failed', 'undelivered', 'skipped_disabled', 'skipped_opt_out', 'skipped_recent', 'skipped_known_contact', 'blocked_pre_send'));
...
grant execute on function public.release_closed_account_phone_numbers(uuid)
  to service_role;
-- Owner-enabled, device-specific Web Push subscriptions. These are service-role
-- managed because browser clients never receive direct table access.
...
-- Existing message, lead, webhook, and Stripe ledgers remain authoritative.
...
-- Content-free evidence for destructive retention and account deletion. This
-- table intentionally has no account foreign key so its evidence survives the
-- tenant row it documents.
...
create index if not exists data_retention_events_target_created_at_idx
  on public.data_retention_events (target_account_id, created_at desc)
  where target_account_id is not null;
...
create policy deny_client_access on public.data_retention_events
  as restrictive for all to anon, authenticated using (false) with check (false);
