...
// --- 4. SMS test status endpoint tenant scoping ---

function smsTestStatusMocks(fetchedMessage, account) {
  return {
    "@/lib/phone": {
...
async function getSmsTestStatus(mocks) {
  const { GET } = await loadTsModule("app/api/sms-test/status/route.ts", mocks);
  return GET(new Request("https://example.com/api/sms-test/status?messageSid=SM_x"));
}

test("sms-test status returns a message involving the tenant's own numbers", async () => {
  const response = await getSmsTestStatus(
    smsTestStatusMocks(
      { sid: "SM_x", from: "+15551234567", to: "+15557654321", status: "delivered", errorCode: null, errorMessage: null, dateUpdated: null },
      TENANT_ACCOUNT,
    ),
  );
...
test("sms-test status refuses another tenant's MessageSid (404, no metadata leak)", async () => {
  const response = await getSmsTestStatus(
    smsTestStatusMocks(
      { sid: "SM_x", from: "+19998887777", to: "+19996665555", status: "delivered", errorCode: 30003, errorMessage: "leaky", dateUpdated: null },
      TENANT_ACCOUNT,
    ),
  );
